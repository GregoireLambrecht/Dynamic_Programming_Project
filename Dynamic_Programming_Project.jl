...
#We assume that a command made day t, arrived at the end of day t. So the command at day t impact the storage cost of the day t. 
...
function policySimulator(πt::Matrix{Int64}, n::Int64, draft::Int64) #πt is the policy, n the binomial parameter of the demand, draft the number of simulations for the monte carlo method
    ...
    for k=1:draft                                                   #Iteration for the monte carlo method
        ...
        for t=1:T
            ...
            STOCK[t+1] = min(20,STOCK[t] + πt[STOCK[t]+1,t] - delivered)*(t<T+1)    #actualisation of the stock
...
function exercice2(n::Int64,T::Int64)                               
    ...
    function evaluateStrat(bin::Binomial{Float64},stock::Int64,command::Int64,t::Int64)::Float64  
        ...
        futureStock::Array{Int64} = fsto.(stock .+ command .- delivered)         #stock at day t+1
        rev = [REVENUE[futureStock[i]+1,t+1] for i=1:(n+1)]                      #maximum expected revenue for the following days
        if ( t!= T)
            return sum((delivered .* 3 - 0.1 .* futureStock .- command .+ rev) .* density.(d))  
        else
            return sum((delivered .* 3) .* density.(d))
        end
    end

    REVENUE[:,T] = [evaluateStrat(Binomial(n,p[T]),i,0,0) for i=0:20]            #Initialisation at t = T

    for t=(T-1):-1:1                                                             #Iteration on days
        bin = Binomial(n,p[t])                                                   #The density of the command                                             
...
REVENUE, STRATEGY = exercice2(7,14)

REVENUE[11,1]
policySimulator(STRATEGY,7,10^6)           #REVENUE[11,1] ∈ confidence interval 